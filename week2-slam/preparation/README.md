...
| 로봇 | TurtleBot3 **Waffle**, `nav2_minimal_tb3_sim`의 공식 모델 |
...
[Nav2 Jazzy Quickstart](https://docs.nav2.org/jazzy/getting_started/quickstart/quickstart/)의 최소 TurtleBot3 시뮬레이션을 사용합니다. 모델·메시·월드·센서·ROS/Gazebo 브리지는 배포 패키지로 설치합니다. `TURTLEBOT3_MODEL` 설정이나 별도 소스 빌드는 필요하지 않습니다. launch에서 Waffle 모델을 지정합니다.
...
preparation/
├── README.md                       # 설치·실행 안내
├── launch/simulation.launch.py     # Gazebo + Waffle + AMCL + Nav2 + RViz
├── maps/tb3_sandbox.{yaml,pgm}      # 월드와 대응하는 공식 지도
├── config/nav2_params.yaml         # Nav2 1.3.13 공식 기본 설정 + 공통 시작 위치
...
설치 스크립트는 `sudo apt-get`으로 Nav2, 최소 TB3 시뮬레이션, `ros_gz`, RViz, TF 도구 등을 설치합니다. 이미 설치한 시스템에서도 사용할 수 있습니다. ROS apt 저장소에서 Jazzy용 패키지를 받으므로 패치 버전은 업데이트 시점에 따라 달라질 수 있습니다. 이 폴더의 지도·Nav2 설정은 1.3.13에서 가져왔으며, 설정에는 공통 시작 위치를 추가했습니다. 설치된 버전은 검사 스크립트로 확인하세요.
...
Gazebo와 RViz가 함께 열리고 Waffle이 **`x=-2.0 m, y=-0.5 m, yaw=0 rad`**에서 생성됩니다. 로봇 앞쪽은 지도의 +X 방향입니다. 시뮬레이션 시간을 켜고 **AMCL에도 같은 시작 위치를 초기값으로 전달**해 Nav2 lifecycle 노드가 시작됩니다. 목표는 자동 전송하지 않습니다.

설치된 최소 TB3 패키지 1.0.1의 URDF에는 잘못된 메시 경로가 있어, 준비용 launch가 존재하는 패키지 메시 경로로 메모리에서 보정한 뒤 `robot_state_publisher`에 전달합니다. 시스템 설치 파일은 수정하지 않습니다.
...
`/clock` 값이 증가하고 `/scan`, `/odom`, `/map`을 수신할 수 있어야 합니다. 초기 위치가 적용된 뒤 `map → base_link` 변환이 조회되고 `/bt_navigator`가 `active`이며 `/navigate_to_pose` action이 보이면 목표를 보낼 준비가 된 것입니다. 노드가 Active라는 사실만으로 목표 도착을 의미하지는 않습니다.
